    ...
    for key, value in dct.items():
        if isinstance(value, str) and key.endswith('_at'):
            try:
                ...
                # Keep as string if not a valid ISO format
                pass
    return dct
        ...
        if self.file_path.exists():
            try:
                with open(self.file_path, 'r', encoding='utf-8') as f:
                    self._data = json.load(f, object_hook=datetime_decoder)
    ...
    def _save(self) -> None:
        ...
        try:
            with open(self.file_path, 'w', encoding='utf-8') as f:
                json.dump(self._data, f, indent=2, ensure_ascii=False, cls=DateTimeEncoder)
        except IOError:
            pass
...
class CachedStorage:
    ...
    def clear(self) -> None:
        ...
        for key in list(self.backend._data.keys()):
...
class StorageManager:
    ...
    def __init__(self, base_path: Optional[Union[str, Path]] = None):
        if base_path is None:
            base_path = Path.home() / '.research_dashboard'
    ...
    def get_backend(self, name: str, backend_type: str = 'json') -> StorageBackend:
        ...
        if name not in self._backends:
            file_path = self.base_path / f"{name}.{'json' if backend_type == 'json' else 'pkl'}"
    ...
    def get_cached_storage(self, name: str, default_ttl: int = 3600) -> CachedStorage:
        """Get or create cached storage."""
        if name not in self._cached_storages:
            backend = self.get_backend(name)
            self._cached_storages[name] = CachedStorage(backend, default_ttl)

...
class UISearchModule:
    ...
    def render(self) -> None:
        ...
        with ui.column().classes("w-full max-w-3xl mx-auto mb-8"):
            with ui.row().classes("w-full items-center gap-2"):
                ...
                ).classes("flex-1").on("keydown.enter", self._handle_enter)
                
                # Bang title display
                self.bang_title_label = ui.label("").classes("text-sm text-gray-500 mr-2")
    ...
    def _handle_key(self, e) -> None:
        ...
        if e.key == "s" and not e.ctrl and not e.alt and not e.shift:
            self.search_input.run_method('focus')
        elif e.key == "Escape":
    ...
    def _perform_search(self, new_tab: Optional[bool] = None) -> None:

"""Dashboard layout component for responsive module arrangement."""
...
class DashboardLayout:
    ...
    def render(self) -> None:
        ...
        with ui.element("div").classes(f"flex-1 {DashboardStyles.PADDING_MD}"):
    ...
    def _render_three_column_layout(self, column_configs) -> None:
        ...
        with ui.row().classes(
            f"w-full {DashboardStyles.GAP_MD} flex-wrap {DashboardStyles.CONTAINER_CENTER}"
        ):
            for column_config in column_configs:
                with ui.column().classes(
                    f"w-full sm:w-1/2 md:w-1/3 lg:w-1/3 mb-4 {DashboardStyles.GAP_SM}"
                ):
    ...
    def _render_one_or_two_column_layout(self, column_configs) -> None:
        ...
        with ui.row().classes(
            ...
            for column_config in column_configs:
                col_class = (
                    "w-full" if self.config.layout.columns == 1 else "w-full sm:w-1/2"
                )
                with ui.column().classes(f"{col_class} mb-4 {DashboardStyles.GAP_SM}"):
                    self._render_column_content(column_config)

...
class TodoModule(ExtendedModule):
    ...
    def _add_todo(self, text: str) -> None:
        ...
        if text.strip():
            new_todo = {
                "id": max([t.get("id", 0) for t in self.todos]) + 1
                if self.todos
                else 1,
                "text": text.strip(),
                "completed": False,
    ...
    def _delete_todo(self, todo_id: int | None) -> None:
        """Delete a todo item."""
    ...
    def _render_todo_items(self) -> None:
        ...
        for todo in self.todos:
            with ui.row().classes(
                "w-full items-center gap-2 p-2 rounded-lg hover:bg-gray-50"
            ):
                # Checkbox
                ui.checkbox(
        ...
        with ui.column().classes("w-full gap-6 max-w-2xl mx-auto"):
            ...
            with ui.row().classes("w-full justify-center gap-6"):
                with ui.card().classes("p-4 text-center"):
                    ui.label(str(len(self.todos))).classes(
                ...
                with ui.card().classes("p-4 text-center"):
                    ui.label(str(len(self.todos) - completed_count)).classes(
